...
@Controller
public class EstoquePedidosFinalizadoController {
	
	@Autowired
	private PedidoRepository pedidoRepository;
	
	@GetMapping("estoque/pedidosFinalizadosEstoque")
	public String pedidosFinalizadosEstoque(Model model) {
		
		List<Pedido> pedidos = pedidoRepository.findByStatus(StatusPedido.FINALIZADO);
		model.addAttribute("pedidos", pedidos);
		
		return ("htmlEstoque/pedidosFinalizadosEstoque");
